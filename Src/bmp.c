...
/* Static functions and variables used internally */
...
/* Stores which color is transparent */
...
BMP_Err_t BMP_parseFile(BMP_t *bmp) {
    ...
    // In this case the fields equals 0
...
BMP_Err_t BMP_blit(BMP_t *bmp, uint32_t _x, uint32_t _y) {
    ...
    for(uint32_t y = 0; y < bmp->h; y ++) {
        ...
        for(uint32_t x = 0; x < bmp->w; x ++) {
            ...
            // Only draw the pixel if no alpha color is set or if the pixel's color 
            //  does not corresponds to the choosen alpha color
